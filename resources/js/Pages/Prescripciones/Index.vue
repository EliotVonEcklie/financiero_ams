<script setup>
    ...
    const title = 'Prescripciones'

    defineProps({ prescripciones: Array })
    ...
    function closeModal(accept) {
        if (accept) {
            axios.delete(route('prescripciones.destroy', softDeleteId.value))
            .then(router.reload())
        }
    ...
    function softDelete(selectedElements) {
        selectedElements.forEach(async x =>
            await axios.delete(route('prescripciones.destroy', x.id))
        )
    ...
    <Layout :title="title">
        <main class="p-4 pt-20 text-gray-900 dark:text-white">
            ...
            <section class="border-t-2 mt-2 pt-6">
                <FwbModal v-if="isShowModal" persistent>
                    ...
                    <template #body>
                        <div class="p-4 md:p-5 text-center">
                            ...
                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Está seguro de querer anular esta prescripción?</h3>
                ...
                <Table
                    empty-message="No hay prescripciones registradas."
                    soft-delete-text="Anular Selección"
                    :headers="['Id', 'Fecha de Elaboración', 'Código Catastro', 'Contribuyente', 'Resolución', 'Estado', 'Acción']"
                    :elements="prescripciones"
                    allow-create allow-soft-delete
                    @create="router.get(route('prescripciones.search'))"
                ...
                >
                    ...
                    <template #row="{ element }">
                        ...
                        <td class="px-3 py-4 flex items-center justify-center">
                            <a :href="route('prescripciones.show', { prescripcione: element.id })" target="_blank" @click.prevent="openPdf" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg px-2.5 py-2.5 text-center">
